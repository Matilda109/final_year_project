...
import { getAllProjects, RepositoryProject, filterProjects } from "@/lib/projectRepository"
import { Loader2 } from "lucide-react"
import { toast } from "sonner"

const PastProjectsTab: React.FC = () => {
  ...
  const applyFilters = (projectsList: RepositoryProject[] = projects) => {
    ...
    if (topicFilter !== "All") {
      filtered = filtered.filter(project => 
        project.tags && project.tags.some(tag => tag.toLowerCase().includes(topicFilter.toLowerCase()))
      );
    }
    ...
    if (supervisorFilter !== "All") {
      filtered = filtered.filter(project => 
        project.supervisor.toLowerCase().includes(supervisorFilter.toLowerCase())
      );
    }
  ...
  useEffect(() => {
    applyFilters();
  }, [searchQuery, yearFilter, topicFilter, supervisorFilter, projects]);
